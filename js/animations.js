...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tl = gsap.timeline({
        scrollTrigger: {
            ...
            scrub: 0.2 // Dikecilkan dari 1.5 supaya animasi nyaris tidak nge-lag dari posisi scroll asli
                       // (lag besar bikin animasi masih "nyusul" pas sticky frame sudah lepas duluan di ujung scroll-track,
                       // kelihatan seperti kegeser dan gambar sempat buram sampai animasinya settle)
        }
    });
    ...
    tl.to('.layer-inner', {
        x: 351, // Geser lebih ke kanan untuk munculkan area samping kosong
        duration: 1.5,
        ease: 'power3.inOut'
    ...
    tl.to('.layer-inner', {
        scale: 6,
        x: -130, // Shift ke kiri untuk fokus ke Yusko (mirror dari Frame-05)
        y: -97, // Fokus portrait
        duration: 1.5,
        ease: 'power3.inOut'
    ...
    tl.to('.layer-inner', {
        x: -351, // Geser ke kiri untuk munculkan area samping kosong (mirror dari Frame-06)
        duration: 1.5,
        ease: 'power3.inOut'
    ...
    tl.to('.layer-inner', {
        y: 490, // Geser ke atas untuk fokus area atap
        duration: 1.5,
        ease: 'power3.inOut'
    ...
    tl.to('.layer-inner', {
        ...
        y: -100, // Geser ke landscape area (seperti Frame-03)
        duration: 2,
        ease: 'power3.inOut'
    ...
    // ==================================================================
    // SCROLL LOCK — cegah "kegeser" lompat ke section Peta sebelum
    // animasi Frame-01 s/d Frame-17 selesai
    // ==================================================================
    // Scroll native yang cepat (fling trackpad, tombol Page Down/End/Spasi)
    // bisa melompati sisa .scroll-track dalam satu langkah, membuat sticky
    // frame lepas duluan sebelum animasi selesai. Selama ScrollTrigger animasi
    // ini masih aktif & progress belum 100%, setiap input scroll dibatasi
    // jaraknya per langkah — begitu progress mencapai 100%, scroll dilepas
    // normal supaya lanjut ke section Peta dst.
    // Catatan: window.scrollTo() terprogram (tombol scrollBottomBtn, mode
    // debug ?scroll=) tidak memicu event wheel/touchmove/keydown, jadi
    // otomatis tidak kena lock ini.
    const zuiTrigger = tl.scrollTrigger;
    const SCROLL_LOCK_STEP = 90; // px per langkah, mendekati 1 notch mouse wheel biasa

    const isZuiLocked = () => !!zuiTrigger && zuiTrigger.isActive && zuiTrigger.progress < 0.999;
    const isTypingTarget = () => ['INPUT', 'TEXTAREA', 'BUTTON', 'A'].includes(document.activeElement?.tagName);

    window.addEventListener('wheel', (e) => {
        if (e.deltaY > 0 && isZuiLocked()) {
            e.preventDefault();
            window.scrollBy(0, Math.min(e.deltaY, SCROLL_LOCK_STEP));
        }
    }, { passive: false });

    let zuiTouchStartY = null;
    window.addEventListener('touchstart', (e) => {
        zuiTouchStartY = e.touches[0].clientY;
    }, { passive: true });

    window.addEventListener('touchmove', (e) => {
        if (zuiTouchStartY === null) return;
        const currentY = e.touches[0].clientY;
        const delta = zuiTouchStartY - currentY; // positif = jari geser ke atas (scroll ke bawah)
        if (delta > 0 && isZuiLocked()) {
            e.preventDefault();
            window.scrollBy(0, Math.min(delta, SCROLL_LOCK_STEP));
        }
        zuiTouchStartY = currentY;
    }, { passive: false });

    window.addEventListener('keydown', (e) => {
        if (isTypingTarget()) return;
        const forwardKeys = ['PageDown', 'End', ' ', 'ArrowDown'];
        if (forwardKeys.includes(e.key) && isZuiLocked()) {
            e.preventDefault();
            window.scrollBy(0, SCROLL_LOCK_STEP);
        }
    });
});
